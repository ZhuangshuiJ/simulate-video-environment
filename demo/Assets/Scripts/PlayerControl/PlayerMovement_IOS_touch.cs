...
public class PlayerMovement_IOS_touch : MonoBehaviour
{
    ...
    private float doubleTouchCurTime = 0;
    #endregion
    ...
    private void OnDoubleTouchHandle()
    {
        // Check if there is any touch input
        if (Input.touchCount == 2)
        {
            ...
            doubleTouchCurTime += Time.deltaTime;
            if (doubleTouchCurTime < 1.0f)
            {
                return;
            }
    ...
    {
        if (isTouching || isShaking || isDoubleTouching)
        {
            Vector3 forward = head.transform.forward;
            Vector3 moveStep = Vector3.zero;
            if (isDoubleTouching)
            {
                moveStep = Vector3.Scale(-forward * moveSpd * Time.deltaTime, new Vector3(1, 0, 1));
            }
            else
            {
                moveStep = Vector3.Scale(forward * moveSpd * Time.deltaTime, new Vector3(1, 0, 1));
